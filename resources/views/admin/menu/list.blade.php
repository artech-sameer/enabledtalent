            ...
            <ol class="dd-list">
                ...
                <li class="dd-item dd3-item" data-id="{{ $menu->slug }}">
                    ...
                    <div class="dd3-content">{{ $menu->name }}
                        ...
                        <button onclick="deleteAjax('{{ route('admin.menu.destroy',$menu->slug)}}',function(){window.location.reload(); })" class="btn btn-link pull-right" style="padding: 0px 6px; margin-top: -2px;">Delete</button>
                        @endcan
                        @endif
                    </div>
                    ...
                    <ol class="dd-list" id="{{ $menu->slug }}">
                        ...
                        <li class="dd-item dd3-item" data-id="{{ $child->slug }}">
                            ...
                            @if($child->grands->count())
                                <ol class="dd-list" id="{{ $child->slug }}">
                                    @foreach($child->grands as $grandChild)
                                    <li class="dd-item dd3-item" data-id="{{ $grandChild->slug }}">
                                        <div class="dd-handle dd3-handle"></div>
                                        <div class="dd3-content">{{ $grandChild->name }}
                                            @can('edit_menu')
                                            <a href="{{ route('admin.menu.edit',$grandChild->slug)}}" class="btn btn-link pull-right" style="padding: 0px 6px; margin-top: -2px;">Edit</a>
                                            @endcan
                                            @can('delete_menu')
                                            <span style="margin:0px 10px 0px 10px" class="pull-right">/</span>
                                            <button onclick="deleteAjax('{{ route('admin.menu.destroy',$grandChild->slug)}}',function(){window.location.reload(); })" class="btn btn-link pull-right" style="padding: 0px 6px; margin-top: -2px;">Delete</button>
                                            @endcan
                                        </div>
                                    </li>
                                    @endforeach
